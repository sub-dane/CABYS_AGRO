...
f_Papa_complemento<-function(directorio,mes,anio){
  ...
  n_fila1=which(Impor== "210300",arr.ind = TRUE)[,"row"]
  ...
  fila1=which(grepl("01510",IPP[,n_col]),arr.ind = TRUE)
  ...
  fila1=which(grepl("01170500",IPC[,1]),arr.ind = TRUE)
  ...
  nuevos_datos=bind_rows(cuadro_importaciones,cuadro_Valor_IPP,cuadro_Valor_IPC)
  return(nuevos_datos)
}
