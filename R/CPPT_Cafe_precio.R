...
f_Cafe_precio_interno_ppt<-function(directorio,mes,anio){
...
Precio_interno <- read_xlsx(paste0(directorio,"/ISE/",anio,"/",carpeta,"/Data/consolidado_ISE/CAFÉ/",archivo),
                             sheet = "P. interno")

columna1=which(grepl("Precio interno",Precio_interno),arr.ind = TRUE)


tamaño=36+mes

Precio=as.data.frame(tail(Precio_interno[,columna1[1]],tamaño))
Precio=as.numeric(Precio$...4)
